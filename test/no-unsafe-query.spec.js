...
ruleTester.run('no-unsafe-query', rule, {
	invalid: [
		{
			code:
				'const column = "*"; foo.query(`SELECT ${column} FROM foobar`);',
			errors: [
				{
					message:
		...
		{
			code:
				'const column = "*"; const query = `SELECT ${column} FROM foobar`; foo.query(query);',
			errors: [
				{
					message:
						'Use the `sql` tagged template literal for raw queries',
		...
		{
			code:
				'const column = "*"; foo.query(foobar`SELECT ${column} FROM foobar`);',
			errors: [
				{
					message:
						'Use the `sql` tagged template literal for raw queries',
		...
		{
			code:
				'const column = "*"; const query = foobar`SELECT ${column} FROM foobar`; foo.query(query);',
			errors: [
				{
					message:
						'Use the `sql` tagged template literal for raw queries',
